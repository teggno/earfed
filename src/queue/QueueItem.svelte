<script lang="ts">
  ...
  import type { EpisodeDescriptionType } from "../userData/episodeTypes";
  ...
  interface Episode {
    showTitle: string;
    episodeTitle: string;
    episodeDescription: { value: string; type: EpisodeDescriptionType };
  ...
  <article>
    ...
    expanding an item that is below the currently expanded item.-->
      <div
        ...
        in:slide={{
          delay: 0,
          duration: episodeDescription.value
            ? Math.max(episodeDescription.value.length / 13, 400)
            : 400,
          easing: delayInTransition ? quadIn : circIn,
        }}
          ...
          {#if episode.durationSeconds}
            <div>
              {episode.positionSeconds
                ? `${Math.round(
                    (episode.durationSeconds - episode.positionSeconds) / 60
                  )} min left`
